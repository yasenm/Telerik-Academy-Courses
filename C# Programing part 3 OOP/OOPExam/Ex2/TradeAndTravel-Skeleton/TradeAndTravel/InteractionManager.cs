...
namespace TradeAndTravel
{
    public class InteractionManager
    {
        ...
        private void HandleDropInteraction(Person actor)
        {
            foreach (var item in actor.ListInventory())
            {
                if (ownerByItem[item] == actor)
                {
                    ...
                    {
                        var newItem = item;
                        newItem.UpdateWithInteraction(string.Empty);
                        if (newItem.Value != 0)
                        {
                            strayItemsByLocation[actor.Location].Add(newItem);
                        }
                    }
                    else
                    {
        ...
        private void HandleGatherInteraction(string[] commandWords, Person actor)
        {
            ...
            if (actor.Location is Forest && armorActive)
            {
                this.AddToPerson(actor, new Wood(commandWords[2]));
            }
            if (actor.Location is Mine && weaponActive)
            {
        ...
        private void HandleCraftInteraction(string[] commandWords, Person actor)
        {
            ...
            if (commandWords[2] == "weapon" && hasIron && hasWood)
            {
        ...
        private void HandleBuyInteraction(string[] commandWords, Person actor)
        {
            ...
            var buyer = personByName[commandWords[3]] as Merchant;
