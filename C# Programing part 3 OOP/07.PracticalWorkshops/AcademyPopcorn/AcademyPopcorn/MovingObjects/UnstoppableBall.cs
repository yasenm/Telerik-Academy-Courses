...
namespace AcademyPopcorn
{
    public class UnstoppableBall : Ball
    {
        ...
        public override void RespondToCollision(CollisionData collisionData)
        {
            ...
            {
                if (collisionData.hitObjectsCollisionGroupStrings[i] == "unpassableBlock" || collisionData.hitObjectsCollisionGroupStrings[i] == "indestructibleBlock" || collisionData.hitObjectsCollisionGroupStrings[i] == "racket")
                {
                    if (collisionData.CollisionForceDirection.Row * this.Speed.Row < 0)
                    {
                        this.Speed.Row *= -1;
                    }
                    if (collisionData.CollisionForceDirection.Col * this.Speed.Row < 0)
                    {
                        this.Speed.Col *= -1;
                    }
                }
